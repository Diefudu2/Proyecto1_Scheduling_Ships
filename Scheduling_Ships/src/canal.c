...
void canal_exit(Canal *c, Ship *s)
{
     ...
     * por interrupcion. No debe contar como cruzado.
     */
    if (found < 0) {
        pthread_mutex_unlock(&c->mutex);
        return;
     ...
     * IMPORTANTE:
     *
     * No despertar colas aqui.
     * El encargado de decidir el lado siguiente es canal_flow_controller().
     *
     * Tampoco reiniciar passed_current para EQUIDAD.
     * Si se reinicia aqui, nunca se alcanza W.
     */
    if (c->state.ship_count == 0) {
        if (c->config.flow_algo == FLOW_TICO) {
            c->state.direction = CANAL_DIR_FREE;
        ...
        /*
         * Para EQUIDAD y LETRERO se conserva la direccion activa.
         * EQUIDAD necesita conservar passed_current hasta llegar a W.
         * LETRERO necesita conservar la direccion del letrero.
         */
    }
...
void *canal_flow_controller(void *arg)
{
    ...
    static CanalDir last_tico_dir = CANAL_DIR_RIGHT;
    ...
    while (g_canal && g_canal == c) {
        ...
        if (c->config.flow_algo == FLOW_EQUIDAD) {
            ...
            if (c->state.ship_count == 0) {
                int has_left  = c->state.queue_left.count  > 0;
                int has_right = c->state.queue_right.count > 0;

                /*
                 * Si no hay barcos, dejar libre.
                 */
                ...
                /*
                 * Si la direccion esta libre, elegir un lado inicial.
                 */
                else if (c->state.direction == CANAL_DIR_FREE) {
                    if (has_left) {
                        c->state.direction = CANAL_DIR_LEFT;
                    } else {
                        c->state.direction = CANAL_DIR_RIGHT;
                    }
                ...
                /*
                 * Si ya pasaron W barcos por el lado actual,
                 * intentar rotar al otro lado.
                 */
                else if (c->state.passed_current >= c->config.equidad_w) {
                    ...
                    /*
                     * Si el otro lado tiene barcos, rotar.
                     * Si no tiene, continuar con el lado actual,
                     * pero reiniciar contador para permitir flujo.
                     */
                    if (dest_has_ships) {
                ...
                /*
                 * Si el lado actual no tiene barcos y el contrario si,
                 * cambiar aunque no se haya llegado a W.
                 */
                if (c->state.direction == CANAL_DIR_LEFT &&
                    !has_left &&
                    has_right) {
                    c->state.direction = CANAL_DIR_RIGHT;
                    c->state.passed_current = 0;
                } else if (c->state.direction == CANAL_DIR_RIGHT &&
                           !has_right &&
                           has_left) {
                    c->state.direction = CANAL_DIR_LEFT;
                    c->state.passed_current = 0;
                ...
                /*
                 * Despertar solo la cola del lado activo.
                 */
                if (c->state.direction == CANAL_DIR_LEFT) {
                    wake_eligible(c, &c->state.queue_left);
                } else if (c->state.direction == CANAL_DIR_RIGHT) {
            ...
            if (c->state.ship_count == 0) {
                ...
                if (has_left && has_right) {
                    /*
                     * TICO:
                     * Si hay barcos en ambos lados, alternar para evitar
                     * preferencia fija por L->R.
                     */
                    if (last_tico_dir == CANAL_DIR_LEFT) {
                        c->state.direction = CANAL_DIR_RIGHT;
                        last_tico_dir = CANAL_DIR_RIGHT;
                        wake_eligible(c, &c->state.queue_right);
                    } else {
                        c->state.direction = CANAL_DIR_LEFT;
                        last_tico_dir = CANAL_DIR_LEFT;
                        wake_eligible(c, &c->state.queue_left);
                    }
                } else if (has_left) {
                    c->state.direction = CANAL_DIR_LEFT;
 ...
 * --------------------------------------------------------- */
static int canal_sleep_ms_interruptible(long total_ms)
{
    long elapsed = 0;
    ...
    return g_scheduler && g_scheduler->active && g_canal;
}
...
void *canal_letrero_thread(void *arg)
{
    ...
    /*
     * Dirección inicial estable.
     */
    CanalDir dir = CANAL_DIR_LEFT;
    canal_set_direction(c, dir);

    while (g_scheduler && g_scheduler->active &&
           g_canal && g_canal == c) {

        /*
         * Mantener el letrero en la misma dirección durante
         * letrero_ms. Esto evita el parpadeo violento.
         */
        if (!canal_sleep_ms_interruptible(c->config.letrero_ms)) {
            break;
        }

        /*
         * Antes de cambiar dirección, esperar a que el canal esté vacío.
         */
        while (g_scheduler && g_scheduler->active &&
               g_canal && g_canal == c) {
            ...
            if (!canal_sleep_ms_interruptible(50)) {
                break;
            }
        }
        ...
        /*
         * Rotar dirección una sola vez por periodo.
         */
        dir = (dir == CANAL_DIR_LEFT)
              ? CANAL_DIR_RIGHT
              : CANAL_DIR_LEFT;
